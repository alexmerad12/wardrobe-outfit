...
**Progress (2026-06-12): C1 ✅ `26baabe` safe-area insets app-wide + white manifest (viewportFit cover; nav/FAB/CTAs/toast/sticky-headers). C7 ✅ `bfce163` Sentry on handled AI failures + `after()` for logAiCall + refine cost (migration `supabase-migration-ai-call-refine.sql` written, NOT applied). C4 ✅ `a3153a5` shared daily-cap helper: consume-after-validation, refund-on-failure, analyze capped 40/day + fetch timeout + maxDuration, bypass parity, loud KV fail-open, clamped 429 counts. Also shipped from beta testing: comfort-day tailored-ban narrowed `5459bc6`, retry-loader blank screen `31ec5a3`. C8 ✅ `f9b36a3` auth trio (OAuth error surfaced on /login + pageshow bfcache reset + eye-toggle padding). C2 partial: weather widget ✅ `b53fe77` (reachable error state, GPS beats slow IP-geo), home+wardrobe error-vs-empty ✅ `7011f96` (loadFailed + retry, shared `common.loadFailed`/`common.retry` keys). C2 ✅ complete (`f76b8ce` suggest/outfits/profile error-vs-empty + form-step gating). C3 P1s ✅ `e1c33ce` (suggest favorite "Saved"/"Wear today", settings "Enregistré", onboarding finish keeps answers + inline errors; shared `common.saveFailed`). C3 ✅ complete (`2a904ce` home/outfits/wardrobe mutations check status, partial-failure surfacing, double-tap guards, try/finally on the today-heart). C5 core ✅ `4432a0a` (today route: archive-before-overwrite in POST, claim-by-delete race guard on all rotations, FK-safe outfit_log with times_worn gated on log success, styling_tip kept in DELETE archive). C5 ✅ complete (`e5ce888` local-day family: IP-timezone cap days via `localDayKey`, locale date labels, noon-anchored share/last-worn dates, packing date-only window, trips local flip). C6 ✅ (`195b3ca` + `734bac3`): normalize raw-image fallback (no more permanent red tiles), zombie-save guard (timeout + retry can't duplicate items), duplicate-pick navigation, storage-object cleanup on item delete, HEIC conversion on change-photo, item-delete status check, eager ~45MB imgly preload removed from add/[id]. Deferred C6 P3s (noted, not fixed): photo-replace old-object cleanup, full AbortController cancellation for cancelAll, bulk-revisit re-queue, normalize-vs-analyze source overwrite. **GROUP C COMPLETE.** Next: Group D i18n sweep, Group E polish, Phase 6 harness.**
...
### Group D — i18n (~1 day)
**Progress: D1+D2 ✅ `5547bb9` (2026-06-13) — duplicate `suggest.styling` shadow-key removed, fr « pièce(s) » counters, localized "No items" strips + translated weather-condition badges via shared `src/lib/weather-condition-label.ts`, try-on locale plumb-through (page sends locale; route uses `buildAnalyzePrompt(locale)` + reasons in user language). D3 ✅ `d16f8d3` — upload-flow harvest (i18n-sentinel pattern via `pendingErrorLabel`, strip CTAs, FAB tips, batch-cap alerts, splash taglines, `html lang` sync). D4 ✅ `2398171` (review page fully localized incl. material/pattern chips via labels maps; password toggle localized + keyboard-reachable + bigger target). D5 ✅ `538b479` (authErrorKey mapping on all five auth surfaces; OAuth sentinel unified). **GROUP D essentially complete** — remaining crumbs folded into Group E: bulk helper copy + raw category enum line, try-on API error strings → codes, prev/next item aria-labels.**
...
**✅ DONE `e4be68e` (2026-06-13).** `npm run test:rules` → `tests/e2e/rule-compliance.spec.ts`: seeded 17-item trap wardrobe, hard `expect()`s on work bans / Comfort-Day bans (incl. the elastic-wide-leg regression) / anchor guarantee / belt completer / complete-base invariant / honest-empty contract; account + wardrobe-size tripwires before wiping; uses the dedicated `e2e-audit-claude@linette.app` user (in `CAP_BYPASS_EMAILS`). `?debug=rules` (admin-gated) returns the B1 telemetry inline. **First live run: 4/4 passed (2.4m, ~$0.30).** Note: `stress-suggest.spec.ts` stays as the broad report-only measurer; the new spec is the enforcement gate. Items 1's "derive rules from config" remains the B4 follow-up.
